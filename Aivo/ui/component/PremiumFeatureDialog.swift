import SwiftUI

// MARK: - Premium Feature Type
enum PremiumFeatureType {
    case song
    case cover
    case lyric
    
    var title: String {
        switch self {
        case .song: return "Generate Song"
        case .cover: return "Generate Cover"
        case .lyric: return "Generate Lyrics"
        }
    }
    
    var hasFreeTrial: Bool {
        let profileManager = ProfileManager.shared
        let remoteConfig = RemoteConfigManager.shared
        guard remoteConfig.enableFreeFirstTime else { return false }
        
        switch self {
        case .song: return !profileManager.hasUsedFreeSongGeneration
        case .cover: return !profileManager.hasUsedFreeCoverGeneration
        case .lyric: return !profileManager.hasUsedFreeLyricGeneration
        }
    }
    
    var detailedDescription: String {
        switch self {
        case .song:
            return "Create original AI-powered songs with custom lyrics, genres, and moods. This feature is exclusive to Premium members and requires credits for each generation."
        case .cover:
            return "Transform any song with AI voice models to create unique cover versions. This feature is exclusive to Premium members and requires credits for each generation."
        case .lyric:
            return "Generate creative, professional lyrics powered by AI for your songs. This feature is exclusive to Premium members and requires credits for each generation."
        }
    }
}

// MARK: - Premium Feature Dialog
struct PremiumFeatureDialog: View {
    let featureType: PremiumFeatureType
    let onGoPremium: () -> Void
    let onTryFree: () -> Void
    let onDismiss: () -> Void
    
    var body: some View {
        ZStack {
            // Dimmed background
            Color.black.opacity(0.6)
                .ignoresSafeArea()
                .onTapGesture { onDismiss() }
            
            // Dialog card
            VStack(alignment: .leading, spacing: 0) {
                // Header: Icon + Title
                HStack(spacing: iPadScaleSmall(12)) {
                    Image(systemName: "crown.fill")
                        .font(.system(size: iPadScale(32)))
                        .foregroundStyle(
                            LinearGradient(
                                colors: [
                                    Color(red: 1.0, green: 0.75, blue: 0.1),
                                    Color(red: 1.0, green: 0.5, blue: 0.0)
                                ],
                                startPoint: .top,
                                endPoint: .bottom
                            )
                        )
                    
                    VStack(alignment: .leading, spacing: 2) {
                        Text("Premium Feature")
                            .font(.system(size: iPadScale(20), weight: .bold))
                            .foregroundColor(.white)
                        
                        Text(featureType.title)
                            .font(.system(size: iPadScale(14), weight: .medium))
                            .foregroundColor(.white.opacity(0.6))
                    }
                    
                    Spacer()
                }
                .padding(.top, iPadScaleSmall(24))
                .padding(.horizontal, iPadScaleSmall(20))
                
                // Description (left-aligned)
                VStack(alignment: .leading, spacing: 8) {
                    Text(featureType.detailedDescription)
                        .font(.system(size: iPadScale(14)))
                        .foregroundColor(.white.opacity(0.75))
                        .lineSpacing(3)
                        .fixedSize(horizontal: false, vertical: true)
                }
                .padding(.top, iPadScaleSmall(14))
                .padding(.horizontal, iPadScaleSmall(20))
                
                // Free trial message (centered)
                if featureType.hasFreeTrial {
                    Text("🎁 You have 1 free trial available!")
                        .font(.system(size: iPadScale(15), weight: .semibold))
                        .foregroundColor(Color(red: 1.0, green: 0.75, blue: 0.1))
                        .multilineTextAlignment(.center)
                        .frame(maxWidth: .infinity)
                        .padding(.top, iPadScaleSmall(14))
                        .padding(.horizontal, iPadScaleSmall(20))
                }
                
                // Buttons
                HStack(spacing: iPadScaleSmall(12)) {
                    // Try Free button (only if free trial available)
                    if featureType.hasFreeTrial {
                        Button(action: onTryFree) {
                            HStack(spacing: 6) {
                                Text("Try Free")
                                    .font(.system(size: iPadScale(15), weight: .semibold))
                                Image("icon_ads")
                                    .renderingMode(.template)
                                    .resizable()
                                    .scaledToFit()
                                    .frame(width: iPadScale(22), height: iPadScale(22))
                            }
                            .foregroundColor(.white)
                            .frame(maxWidth: .infinity)
                            .frame(height: iPadScale(44))
                            .background(
                                RoundedRectangle(cornerRadius: iPadScale(10))
                                    .stroke(Color.white.opacity(0.4), lineWidth: 1.5)
                            )
                        }
                    }
                    
                    // Go Premium button
                    Button(action: onGoPremium) {
                        Text("Go Premium")
                            .font(.system(size: iPadScale(15), weight: .bold))
                            .foregroundColor(.black)
                            .frame(maxWidth: .infinity)
                            .frame(height: iPadScale(44))
                            .background(
                                RoundedRectangle(cornerRadius: iPadScale(10))
                                    .fill(
                                        LinearGradient(
                                            colors: [
                                                Color(red: 1.0, green: 0.75, blue: 0.1),
                                                Color(red: 1.0, green: 0.5, blue: 0.0)
                                            ],
                                            startPoint: .leading,
                                            endPoint: .trailing
                                        )
                                    )
                            )
                    }
                }
                .padding(.horizontal, iPadScaleSmall(20))
                .padding(.top, iPadScaleSmall(20))
                .padding(.bottom, iPadScaleSmall(24))
            }
            .frame(maxWidth: iPadScale(320))
            .background(
                RoundedRectangle(cornerRadius: 20)
                    .fill(Color(white: 0.12))
            )
            .overlay(
                RoundedRectangle(cornerRadius: 20)
                    .stroke(Color.white.opacity(0.1), lineWidth: 1)
            )
            .shadow(color: .black.opacity(0.5), radius: 30, x: 0, y: 10)
        }
    }
}

// MARK: - Preview
#Preview {
    ZStack {
        AivoSunsetBackground()
        PremiumFeatureDialog(
            featureType: .song,
            onGoPremium: {},
            onTryFree: {},
            onDismiss: {}
        )
    }
}
